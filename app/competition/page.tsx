...
function Tick({ yes }: { yes: boolean | null }) {
  if (yes === true)  return <Check size={15} className="text-[#0066ff]" strokeWidth={2.5} />;
  if (yes === false) return <X     size={15} className="text-red-400"   strokeWidth={2.5} />;
  return <Minus size={15} className="text-[#9b9b9b]" strokeWidth={2.5} />;
}
  ...
  return (
    <div className="bg-white text-[#0a0a0a]">
      ...
      <section className="max-w-6xl mx-auto px-6 sm:px-8 pt-20 pb-16">
        <Reveal>
          <p className="text-[11px] font-semibold tracking-widests uppercase text-[#0066ff] mb-4">Competitive Analysis</p>
          <h1 className="text-4xl sm:text-5xl font-bold leading-[1.08] tracking-tight mb-5 max-w-3xl">
            Who else is in this space.<br />
            <span className="text-[#9b9b9b]">And why we win.</span>
          </h1>
      ...
      {/* Feature matrix */}
      <section className="border-t border-[#e8e8e8] bg-[#f7f7f7] py-16 md:py-20">
        <div className="max-w-6xl mx-auto px-6 sm:px-8">
          <Reveal className="mb-10">
            <p className="text-[11px] font-semibold tracking-widests uppercase text-[#0066ff] mb-4">Feature Matrix</p>
              ...
              <table className="w-full min-w-[600px]">
                <thead>
                  <tr className="bg-[#0a0a0a]">
                    ...
                    <th className="text-center text-[11px] font-semibold tracking-widests uppercase text-[#6b9bff] px-5 py-3.5">Clarix</th>
                ...
                <tbody>
                  {matrix.map((row, i) => (
                    <tr key={row.feature} className={i % 2 === 0 ? "bg-white" : "bg-[#f7f7f7]"}>
                      <td className="px-5 py-3 text-[13px] font-medium text-[#0a0a0a] border-b border-[#e8e8e8]">{row.feature}</td>
                      <td className="px-5 py-3 text-center border-b border-[#e8e8e8]"><div className="flex justify-center"><Tick yes={row.clarix} /></div></td>
                      <td className="px-5 py-3 text-center border-b border-[#e8e8e8]"><div className="flex justify-center"><Tick yes={row.chatgpt} /></div></td>
                      <td className="px-5 py-3 text-center border-b border-[#e8e8e8]"><div className="flex justify-center"><Tick yes={row.copilot} /></div></td>
                      <td className="px-5 py-3 text-center border-b border-[#e8e8e8]"><div className="flex justify-center"><Tick yes={row.consultants} /></div></td>
                    </tr>
                  ))}
                </tbody>
              </table>
            </div>
            <p className="text-[12px] text-[#9b9b9b] mt-3">— = partial or context-dependent</p>
          </Reveal>
        </div>
      </section>
      ...
      <section className="max-w-6xl mx-auto px-6 sm:px-8 py-20 md:py-24">
        <Reveal className="mb-14">
          <p className="text-[11px] font-semibold tracking-widests uppercase text-[#0066ff] mb-4">The Landscape</p>
          <h2 className="text-3xl sm:text-4xl font-bold tracking-tight">Every major player, honestly assessed.</h2>
        </Reveal>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {competitors.map((c, i) => (
            <Reveal key={c.name} delay={i * 60}>
              <div className="border border-[#e8e8e8] rounded-xl p-6 h-full hover:border-[#d0d0d0] hover:shadow-sm transition-all">
                <div className="flex items-start justify-between mb-4">
                  <div>
                    <h3 className="font-bold text-[#0a0a0a] text-[16px]">{c.name}</h3>
                    <p className="text-[12px] text-[#9b9b9b] mt-0.5">{c.type} · {c.price}</p>
                  </div>
                  <div className="w-2 h-2 rounded-full mt-2 flex-shrink-0" style={{ background: c.color }} />
                </div>
                <p className="text-[#6b6b6b] text-[13px] leading-relaxed mb-5">{c.summary}</p>
                <div className="grid grid-cols-2 gap-4 mb-5">
                  <div>
                    <p className="text-[10px] font-semibold tracking-widests uppercase text-[#0066ff] mb-2">Strengths</p>
                    <ul className="space-y-1.5">
                      {c.strengths.map((s) => (
                        <li key={s} className="flex gap-1.5 text-[12px] text-[#6b6b6b]">
                          <Check size={11} className="text-[#9b9b9b] flex-shrink-0 mt-0.5" />
                          {s}
                        </li>
                      ))}
                    </ul>
                  ...
                  <div>
                    ...
                    <ul className="space-y-1.5">
                      {c.weaknesses.slice(0, 4).map((w) => (
                        <li key={w} className="flex gap-1.5 text-[12px] text-[#6b6b6b]">
                ...
                <div className="border-t border-[#e8e8e8] pt-4">
                  <p className="text-[12px] text-[#0a0a0a] font-semibold">Verdict: <span className="font-normal text-[#6b6b6b]">{c.verdict}</span></p>
                </div>
              </div>
            </Reveal>
          ))}
      ...
      <section className="border-t border-[#e8e8e8] bg-[#f7f7f7] py-20 md:py-24">
        <div className="max-w-6xl mx-auto px-6 sm:px-8">
          <Reveal className="mb-14">
            <p className="text-[11px] font-semibold tracking-widests uppercase text-[#0066ff] mb-4">Our Edge</p>
          ...
          <Reveal stagger className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {ourAdvantages.map((a, i) => (
              <div key={a.title} className="bg-white border border-[#e8e8e8] rounded-xl p-6">
                <span className="font-mono text-[11px] text-[#9b9b9b] mb-4 block">0{i + 1}</span>
                <h3 className="font-bold text-[#0a0a0a] text-[14px] mb-2">{a.title}</h3>
                <p className="text-[#6b6b6b] text-[13px] leading-relaxed">{a.desc}</p>
              </div>
            ))}
          </Reveal>
        </div>
      ...
      <section className="max-w-4xl mx-auto px-6 sm:px-8 py-20 md:py-24">
        <Reveal>
          <p className="text-[11px] font-semibold tracking-widests uppercase text-[#0066ff] mb-4">The Gap We Own</p>
          ...
          <p className="text-[#6b6b6b] text-[16px] leading-relaxed mb-10">
            Clarix at <strong className="text-[#0a0a0a]">$100–$250/mo</strong> occupies a position no one else is selling: <strong className="text-[#0a0a0a]">white-glove, persistent, proactive, and affordable</strong>. That gap isn&apos;t an accident — it exists because nobody built the operational playbook to serve it at this price. We did.
          </p>
